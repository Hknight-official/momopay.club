...
class napthe {
	...
	public function __construct ($key){ // nhập dữ liệu key api khi gọi class
		self::$key = $key;
	}
 # apiv2 thesieutoc
	...
	// 'content' = $content  
	// )
	public function get_card_v2($method){ // hàm gửi card lên api
	...
	public function ValidateCallback($out) { //Hàm kiểm tra callback từ sever
		if(isset(
			$out['status'],
			$out['serial'],
			$out['pin'],
